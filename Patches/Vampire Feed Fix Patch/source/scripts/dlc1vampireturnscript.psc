...
bool Property isAnimVampAnimRunning = false auto
...
ReferenceAlias Property NewVampire6 auto
ReferenceAlias Property NewVampire7 auto
ReferenceAlias Property NewVampire8 auto
ReferenceAlias Property NewVampire9 auto
ReferenceAlias Property NewVampire10 auto
ReferenceAlias Property NewVampire11 auto
ReferenceAlias Property NewVampire12 auto
ReferenceAlias Property NewVampire13 auto
...
; 				Debug.Trace(self + "CompleteChange() making DLC1RV07 Candidate, setting DLC1RV07 stage 50, and adding to DLC1RV07ThankFaction")
			...
			;Let everyone know the actor is now a vampire
			ActorToTurn.SendVampirismStateChanged(true)
		else
...
{Called by DLC1VampireTurnPlayerScript when player changes location}
	...
	CompleteChange(NewVampire6)
	CompleteChange(NewVampire7)
	CompleteChange(NewVampire8)
	CompleteChange(NewVampire9)
	CompleteChange(NewVampire10)
	CompleteChange(NewVampire11)
	CompleteChange(NewVampire12)
	CompleteChange(NewVampire13)
EndFunction
...
ReferenceAlias Function GetNextAlias()
	...
	elseif NewVampire6.GetReference() == false
		ReferenceAliasToReturn = NewVampire6
	elseif NewVampire7.GetReference() == false
		ReferenceAliasToReturn = NewVampire7
	elseif NewVampire8.GetReference() == false
		ReferenceAliasToReturn = NewVampire8
	elseif NewVampire9.GetReference() == false
		ReferenceAliasToReturn = NewVampire9
	elseif NewVampire10.GetReference() == false
		ReferenceAliasToReturn = NewVampire10
	elseif NewVampire11.GetReference() == false
		ReferenceAliasToReturn = NewVampire11 
	elseif NewVampire12.GetReference() == false
		ReferenceAliasToReturn = NewVampire12
	elseif NewVampire13.GetReference() == false
		ReferenceAliasToReturn = NewVampire13
	else
...
; 	Debug.Trace(self + "ReceiveHarkonsGift() IsSeranaGiving ==" + IsSeranaGiving)
	...
	isAnimVampAnimRunning = false

	If (!RegisterForAnimationEvent(GiftGiver, animEventWaitFor))
		Debug.Notification("Attacker Failed to register for" + animEventWaitFor)
	EndIf	
...
; 		Debug.Trace(self + "ReceiveHarkonsGift() pa_VampireLordChangePlayer animPlayed: " + animPlayed)
	...
	WaitForAnimation(animEventWaitFor, GiftGiver)
...
Function WaitForAnimation(string animEvent, actor attacker, float TimeoutDuration = 15.0)
	Float elapsedTime = 0.0
	Float interval = 0.1

	While (elapsedTime < TimeoutDuration)
		If isAnimVampAnimRunning
			;Debug.Notification("Animation finished within timeout.")
			Return
		EndIf
	...
	; Debug.Notification("Animation Timeout, exiting...")
	UnregisterForAnimationEvent(attacker, animEvent)
EndFunction

Event OnAnimationEvent(ObjectReference akSource, string asEventName)
	if asEventName == animEventWaitFor
		; Debug.Notification("event: "+asEventName+" name: "+akSource.getbaseobject().getname())
		isAnimVampAnimRunning = true
	endIf
endEvent
