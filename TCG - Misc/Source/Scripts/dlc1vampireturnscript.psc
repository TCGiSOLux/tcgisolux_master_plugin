...
Function CompleteChange(ReferenceAlias AliasToTurn)
	...
	if ActorToTurn
		...
		;UDGP 2.0.1 - Needed to add a check for the actor being in a None location as it appears at least some targets can be and the function aborts if this is the case.
		Location ATTLoc = ActorToTurn.GetCurrentLocation()
		
		if ATTLoc == None || (PlayerRef.IsInLocation(ATTLoc) == false)
...
;*** SETTING RACE CAUSED PROBLEMS, we are now only giving them red eyes.
			...
			ActorToTurn.SendVampirismStateChanged(true) ;Official Patch 1.5.3
		else
...
{Called OnLoad() by DLC1VampireTurnRedEyes, and by MakeAliasesEyesRed()}
	...
	;UDGP 1.2.2 added 3D load check due to Papyrus errors on startup if you're not in the location.
