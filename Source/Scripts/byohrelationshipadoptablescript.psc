...
Keyword property LinkCustom05 Auto			;Link to the child's guardian, if any.
Keyword property LinkCustom06 Auto			;Link to the child's guardian, if any.
...
Function TriggerStewardCourierEvent()
	if (BYOHAdoption_PlayerOwnsAnyHouse)
		Location homeCityLoc = None ;USSEP 4.3.5 Bug #35633
		...
		ElseIf ((HousePurchase as HousePurchaseScript).RiftenHouseVar >= 1) ;USSEP 4.3.5 Bug #35633
			homeCityLoc = RiftenLocation
		EndIf
		
		;Trigger the event.
		;USSEP 4.3.5 Bug #35633 - Call previously assumed Riften if no other condition passed.
		;Since this runs at the start of a new game, the courier delivering you anything house related should not happen. [DLC Start only applies to Skyrim LE, not SE]
		if homeCityLoc != None
...
Function UpdateOrphanablesStatus()
	...
	if (Self.GetStage() == 0)
		...
		ObjectReference guardian03
		ObjectReference guardian04
		int i = 0
		...
		While (i < ((Self as Quest) as BYOHRelationshipAdoptableAccessor).OrphanableChildren.Length)
			...
			if (child != None)	
				...
				if (child.GetFactionRank(BYOHRelationshipAdoptableFaction) < 0)
					...
					guardian03 = child.GetLinkedRef(LinkCustom05)
					guardian04 = child.GetLinkedRef(LinkCustom06)
					...
					if ((father == None || (father as Actor).IsDead() || (father as Actor).IsCommandedActor()) && \
						...
						(guardian03 == None || (guardian03 as Actor).IsDead() || (guardian03 as Actor).IsCommandedActor()) && \
						(guardian04 == None || (guardian04 as Actor).IsDead() || (guardian04 as Actor).IsCommandedActor()))
