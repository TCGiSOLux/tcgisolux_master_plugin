Scriptname TCGCheckVarNPCEnableObjectsCellOwner extends ObjectReference  
...
Faction Property TCGFaction AUTO
Cell Property CellToChange AUTO
	    ...
	    if (triggerRef == (PlayerREF as ObjectReference))
			if TCGPlayerLevelGlobalVariable.GetValueInt() > 0 && NPCREF.IsDead() == true
                If currentdayspassed >= TCGPlayerLevelGlobalVariable.GetValueInt() + DaysPassedRequirements
                    ...
                    CellToChange.SetFactionOwner(TCGFaction)
                    GoToState("Done")
                EndIf
            Endif
        Endif
