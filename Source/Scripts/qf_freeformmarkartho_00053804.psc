...
Alias_Calcelmo.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 1)
Alias_Nimhe.TryToDisable() ; Added by USKP 2.0 to stop Nimhe from respawning.

;Guard shouldn't stop the player from entering the museum
MarkarthWizardsQuarter01.SetPublic()
If DialogueMarkarth.GetStageDone(120) == 0
...
If Alias_Nimhe.GetActorReference().IsDead() == True
  SetStage(15)
EndIf
;END CODE
EndFunction
